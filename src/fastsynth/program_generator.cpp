...
bool is_linear(std::string operand_string)
{
  return (
    operand_string.find("bvshl") == std::string::npos &&
    operand_string.find("bvlshr") == std::string::npos &&
    operand_string.find("bvand") == std::string::npos &&
    operand_string.find("bvxor") == std::string::npos &&
    operand_string.find("bvor") == std::string::npos &&
    operand_string.find("bvudiv") == std::string::npos &&
    operand_string.find("bvmul") == std::string::npos);
}
  ...
  operationt &op)
{
  ...
  if(op.name != "ite" && (operand1.contains_if || operand2.contains_if))
    return true;

  // don't use duplicate operands unless with bvadd or bvmul
  if(op.name != "bvadd" && op.name != "bvmul")
  {
    if(op1_idx == op2_idx)
      return true;
  }
  ...
  if(
    op.name == "bvadd" || op.name == "bvor" || op.name == "bvxor" ||
    op.name == "bvand" || op.name == "=" || op.name == "bvult" ||
    op.name == "bvule" || op.name == "bvmul")
    if(op1_idx >= op2_idx)
      return true;
  ...
  if(op.name == "bvsub")
  {
    ...
    {
      if(
        (operand1.string.find("|synth::parameter" + std::to_string(i))) !=
          std::string::npos &&
        is_linear(operand1.string))
      {
        if(
          operand2.string.find("|synth::parameter" + std::to_string(i)) !=
            std::string::npos &&
          is_linear(operand1.string))
        {
          return true;
    ...
    {
      if(
        (operand1.string.find("|constant_value" + std::to_string(i))) !=
          std::string::npos &&
        is_linear(operand1.string))
      {
        if(
          operand2.string.find("|constant_value" + std::to_string(i)) !=
            std::string::npos &&
          is_linear(operand2.string))
          return true;
      }
...
{
  std::vector<std::string> bit_vecs = {
    "bvadd", "bvsub", "bvshl", "bvand", "bvor", "bvxor", "bvlshr"};

  if(use_mult_and_divide)
  {
    bit_vecs.push_back("bvmul");
    bit_vecs.push_back("bvudiv");
  }

  static const std::vector<std::string> bool_returns = {"bvule", "bvult", "="};
#ifdef DEBUG
  std::cout << "Bitvector operations: ";
  for(const auto &bv : bit_vecs)
    std::cout << bv << " ";
  std::cout << "\nBool operations: ";
  for(const auto &bool_op : bool_returns)
    std::cout << bool_op << " ";
  std::cout << std::endl;
#endif
...
program_generatort::instructiont
program_generatort::get_ite_operands(program_generatort::operationt &op)
{
  PRECONDITION(op.name == "ite");

  std::uniform_int_distribution<unsigned int> bitvec_operand_dis(
    0, bitvec_operands.size() - 1);
  ...
  program_generatort::operationt &op)
{
  ...
  while(!found_operands)
  {
    ...
    found_operands =
      !discard_operands(op1, operand_1_index, op2, operand_2_index, op);
  }
...
program_generatort::instructiont
program_generatort::get_binary_bool_operands(program_generatort::operationt &op)
{
  instructiont res;
  res.length = 1;
  res.contains_if = false;
...
program_generatort::instructiont program_generatort::get_random_instruction(
  program_generatort::operator_typet type,
  bool not_if,
  bool not_shift)
{
  operationt op;
  ...
  {
    std::uniform_int_distribution<unsigned int> bitvec_operators_dis(
      0, bitvec_return_operations.size() - 1);

    op = bitvec_return_operations[bitvec_operators_dis(gen)];
    // discard if
    std::size_t loop_iter = 0;
    while((not_if && op.name == "ite") || (not_shift && op.name == "bvlshr") ||
          (not_shift && op.name == "bvshl"))
    {
      loop_iter++;
      if(loop_iter > LOOPLIMIT)
      {
        std::cerr << "UNABLE TO FIND OPERATOR IN " << LOOPLIMIT
                  << "iterations \n";
  ...
  {
    std::uniform_int_distribution<unsigned int> bool_operators_dis(
      0, bool_return_operations.size() - 1);
  ...
  for(std::size_t p = 0; p < number_of_programs; p++)
  {
    ...
    else
    {
      while(instruction_length < program_size - 1)
      {
        ...
        {
          ins = get_random_instruction(
            operator_typet::BITVEC_RETURN,
      ...
      if(bool_return)
      {
        ...
        instructiont final_ins =
          get_random_instruction(operator_typet::BOOL_RETURN, true, true);
