...
int generate_programs(const cmdlinet &cmdline, std::size_t number_of_programs)
{
  ...
  message.status() << "Number of possible programs "<< program_generator.solver.number_of_options<<messaget::eom;;
...
// this is our default verbosity
  ...
  std::size_t number_of_programs=std::stol(
      cmdline.get_value("generate-N-programs"));

  if(cmdline.isset("number-of-constants"))
  {
    number_of_constants=std::stoul(
          cmdline.get_value("number-of-constants"));
  }
  else
  {
  ...
  if(cmdline.isset("program-size"))
  {
    program_size=std::stoul(
        cmdline.get_value("program-size"));
    if(program_size==0)
    {
      message.warning() <<
          "program size of zero is impossible" << messaget::eom;
      return;
    }
    if(program_size==2)
    {
  ...
  if(cmdline.isset("number-of-params"))
    number_of_params=std::stoul(
        cmdline.get_value("number-of-params"));
  else
