...
void bound_array_exprs_old(exprt &expr, std::size_t length)
{
  for (exprt &op : expr.operands())
    bound_array_exprs_old(op, length);
    ...
    problemt &problem, cegist &cegis, std::size_t &array_size)
{
  ...
  {
    bound_array_exprs_old(c, array_size);
    unwind_quantifiers(c);
  }
  for (auto &s : problem.side_conditions)
  {
    bound_array_exprs_old(s, array_size);
  ...
  {
    exprt copy = *it;
    bound_array_exprs_old(copy, array_size);
    if (copy != *it)
    {
